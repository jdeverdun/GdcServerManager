...
(id integer NOT NULL AUTO_INCREMENT, nom varchar(70) UNIQUE,constraint project_pk primary key (id));
...
(id integer NOT NULL AUTO_INCREMENT, nom varchar(50),id_project integer,
constraint patient_pk primary key (id),
constraint uprojet_nom unique(nom,id_projet),
constraint projectpatient_fk foreign key (id_project) references Project(id) on delete cascade);

create table AcquisitionDate
(id integer NOT NULL AUTO_INCREMENT, nom varchar(50),id_project integer,id_patient integer,
constraint acquisitiondate_pk primary key (id),
constraint upatient_projet_nom unique(nom,id_projet,id_patient),
constraint patientacqdate_fk foreign key (id_patient) references Patient(id) on delete cascade);

create table Protocol
(id integer NOT NULL AUTO_INCREMENT, nom varchar(70),id_project integer,id_patient integer,id_acqdate integer,
constraint protocol_pk primary key (id),
constraint uacqdata_patient_projet_nom unique(nom,id_projet,id_patient,id_acqdate),
constraint acqdateprotocol_fk foreign key (id_acqdate) references AcquisitionDate(id) on delete cascade);

create table Serie
(id integer NOT NULL AUTO_INCREMENT, nom varchar(70),hasnifti tinyint(1),id_project integer,id_patient integer,id_acqdate integer,id_protocol integer,
constraint serie_pk primary key (id),
constraint uprot_acqdata_patient_projet_nom unique(nom,id_projet,id_patient,id_acqdate,id_protocol),
constraint protocolserie_fk foreign key (id_protocol) references Protocol(id) on delete cascade);

create table DicomImage
(id integer NOT NULL AUTO_INCREMENT, nom varchar(50),id_project integer,id_patient integer,id_acqdate integer,id_protocol integer,id_serie integer,
constraint dicomimage_pk primary key (id),
constraint uprot_acqdata_patient_projet_nom unique(nom,id_projet,id_patient,id_acqdate,id_protocol,id_serie),

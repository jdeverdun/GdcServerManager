...
public class DICOM extends ImagePlus implements PlugIn {
	...
	*/
	...
	/**
	 * Fonction maison pour recuperer le header d'un dicom
	 * @param fil
	 * @return
	 */
	public String getInfo(Path fil){
		DicomDecoder dd = new DicomDecoder(fil.getParent().toString(), fil.getFileName().toString());
		...
		if (fi!=null && fi.width>0 && fi.height>0 && fi.offset>0) {
			...
			return (String) dd.getDicomInfo();
		} else if (showErrors){
			try {
				dd.inputStream.close();
			} catch (IOException e) {
				e.printStackTrace();
			}
		}

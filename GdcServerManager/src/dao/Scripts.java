...
public class Scripts {
	...
	public static String[] getCreateUserViews(User u){
		...
		script[6] = "GRANT SELECT ON `"+SQLSettings.DATABASE_NAME+"`.`Project_"+u.getId()+"` TO '"+u.getLogin()+"'@'';";
		script[7] = "GRANT SELECT ON `"+SQLSettings.DATABASE_NAME+"`.`Patient_"+u.getId()+"` TO '"+u.getLogin()+"'@'';";
		script[8] = "GRANT SELECT ON `"+SQLSettings.DATABASE_NAME+"`.`AcquisitionDate_"+u.getId()+"` TO '"+u.getLogin()+"'@'';";
		script[9] = "GRANT SELECT ON `"+SQLSettings.DATABASE_NAME+"`.`Protocol_"+u.getId()+"` TO '"+u.getLogin()+"'@'';";
		script[10] = "GRANT SELECT ON `"+SQLSettings.DATABASE_NAME+"`.`DicomImage_"+u.getId()+"` TO '"+u.getLogin()+"'@'';";
		script[11] = "GRANT SELECT ON `"+SQLSettings.DATABASE_NAME+"`.`NiftiImage_"+u.getId()+"` TO '"+u.getLogin()+"'@'';";
		
		script[12] = "flush privileges;";

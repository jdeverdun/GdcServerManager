...
public class Nifti_Writer  {
	...
	public Nifti_Writer(int format){
		try{
			setOutput_type(format);
		}catch(Exception e){
			e.printStackTrace();
		}
	}
	public void write(ImagePlus imp, Path path, int format) {
		...
		save(imp, directory.toString(), name.toString());
	...
	public boolean save(ImagePlus imp, String directory, String name) {
		...
		try {
			...
			System.err.println("Nifti_Writer: "+ e.getMessage());
			return false;	
		}
	} 
	...
	private void writeHeader( ImagePlus imp, DataOutputStream output, int type ) throws IOException {
		...
		if (type != ANALYZE_7_5) { 
			...
			if (orientProp instanceof CoordinateMapper[] ) { 
				CoordinateMapper[] mapper = (daemon.tools.nifti.CoordinateMapper[] ) orientProp;
	...
	boolean is16BitSigned(ImagePlus imp) { 
		...
		public void setOutput_type(int output_type) throws Exception {
			if(output_type >= 0 & output_type <= 2)
				this.output_type = output_type;
			else
				throw new Exception("NiftiWriter : Output type unknow");
		}
		
		/**
		 * Convertit une image dicom en nifti (format désiré)
		 * @param dicomPath
		 * @param niftiPath
		 * @param format
		 */
		public void dicomToNifti(Path dicomPath, Path niftiPath, int format) {
			write(new ImagePlus(dicomPath.toString()),niftiPath,format);
		}
